// features/app_section/view/screens/bottom_navigator_ui.dart
...
class BottomNavUI extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return MultiBlocProvider(
      providers: [
        ...
        BlocProvider(create: (context) => _createCartCubit()),
      ],
            ...
            bottomNavigationBar: BottomNavigationBar(
              ...
              items: [
                BottomNavigationBarItem(
                  icon: NavIcon(path: AppAssets.homeIconSvg, index: 0, currentIndex: cubit.currentIndex),
                  label: "Home",
                ),
                BottomNavigationBarItem(
                  icon: NavIcon(path: AppAssets.cartIconSvg, index: 1, currentIndex: cubit.currentIndex),
                  label: "Cart",
                ),
                BottomNavigationBarItem(
                  icon: NavIcon(path: AppAssets.favIconSvg, index: 2, currentIndex: cubit.currentIndex),
                  label: "Favourite",
                ),
                BottomNavigationBarItem(
                  icon: NavIcon(path: AppAssets.accountIconSvg, index: 3, currentIndex: cubit.currentIndex),
                  label: "Account",
                ),
              ],
            ),
  ...
  static CartCubit _createCartCubit() {
    final cartDataSource = CartDataSourceImp();
    final cartRepo = CartRepoImp(cartDataSource: cartDataSource);
